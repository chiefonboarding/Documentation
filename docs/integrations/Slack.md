...
5. Go to "Event Subscription" and enable it. Use `https://YOURDOMAIN/api/slack/bot` as the request url. Under 'Subscribe to bot events' add `message.im` and `team_join`. Under "Subscribe to event on behalf of users" and add `im_created` and `message.im`.
